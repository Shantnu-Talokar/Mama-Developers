 ...
 *****************************************************************/
                ...
                memeBtn.onclick = async () => {
                    ...
                    const prompt = `
You are a meme writer. Create a funny two-line meme about:
"${topic}"

Return ONLY minified JSON with keys:
  top   – text for the top of the meme
  bottom– text for the bottom
  category – one of [before_after, comparison_vs, reaction, generic]

Example:
{"top":"Before coding","bottom":"After debugging","category":"before_after"}
  `.trim();
                    ...
                    try {
                        const resp = await fetch("https://api.cohere.ai/v1/generate", {
                            ...
                            body: JSON.stringify({ model: "command-r", prompt, max_tokens: 60, temperature: 0.8 })
                        });
                        const { generations } = await resp.json();
                        ({ top: topText, bottom: bottomText, category } = JSON.parse(generations[0].text.trim()));
                    } catch (_) {
                        /* fallback to old prompt style (two lines “Top:” / “Bottom:”) */
                        const legacy = await fetch("https://api.cohere.ai/v1/generate", {
                            method: "POST",
                            headers: {
                                Authorization: "Bearer zXH8KUSA3ncfZcxvIAZx5boAlGlTirN6LJmp706Q",
                                "Content-Type": "application/json"
                            ...
                            body: JSON.stringify({
                                model: "command", prompt: `Top and bottom meme lines about ${topic}.`,
                                max_tokens: 40, temperature: 0.9
                            })
                        }).then(r => r.json());
                        const lines = legacy.generations?.[0]?.text?.split("\n") || [];
                        topText = lines.find(l => l.toLowerCase().startsWith("top:"))?.replace(/top:/i, "").trim() || "Debugging for hours";
                        bottomText = lines.find(l => l.toLowerCase().startsWith("bottom:"))?.replace(/bottom:/i, "").trim() || "Then it was a semicolon 😭";
                        category = detectCategory(topText, bottomText);
                    }
