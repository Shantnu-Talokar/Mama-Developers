    ...
    btn.onclick = async () => {
        ...
        try {
            ...
            modBtn.onclick = () => {
                ...
                /*****************************************************************
 *  MEME GENERATOR – smarter template selection
 *****************************************************************/
                const templateCategories = {
                    //               🔗 common “two-panel” / “before-after”
                    before_after: ["61585", "161865971", "112126428", "247375501", "28251713"],
                    //               🔗 explicit A vs B or list-style compare
                    comparison_vs: ["217743513", "141787839", "203373420", "169114136", "423511868"],
                    //               🔗 top = situation, bottom = reaction/punch-line
                    reaction: ["4087833", "124055727", "181913649", "134797956", "438680"],
                    //               🔗 one-panel fallback
                    generic: ["61579", "61533", "29562797", "109765", "100777631"]
                };

                /* ---- fallback heuristic if Cohere doesn’t return a category ---- */
                
                function chooseTemplate(category) {
                    const pool = templateCategories[category] || templateCategories.generic;
                    return pool[Math.floor(Math.random() * pool.length)];
                }

                /* ------------ meme button UI (unchanged layout) --------------- */
                ...
                memeBtn.style.cssText =
                    "margin-top:10px;margin-left:8px;padding:6px 12px;border:none;background:#ff5722;color:white;border-radius:4px;cursor:pointer;";
                ...
                memeBtn.onclick = async () => {
                    ...
                    /* ---------- 1️⃣ Ask Cohere for caption + category ------------- */
 ...
 *****************************************************************/
                ...
                /* ---- fallback heuristic if Cohere doesn’t return a category ---- */
                function detectCategory(top, bottom) {
                    const loTop = top.toLowerCase(), loBottom = bottom.toLowerCase();
                    if ((loTop.includes(" vs ") || loTop.includes(" versus ")) ||
                        /.+\s+vs\.?\s+.+/i.test(top)) return "comparison_vs";
                    if (/(before|then)/i.test(loTop) && /after/i.test(loBottom)) return "before_after";
                    if (/^when\b/i.test(loTop) || loBottom.endsWith("😭") || loBottom.length < loTop.length / 2)
                        return "reaction";
                    return "generic";
                }
                function chooseTemplate(category) {
                ...
                memeBtn.onclick = async () => {
                    ...
                    const topic = document.querySelector("h1").textContent.trim();
                    const prompt = `
You're meme generator. Give me two lines that are related to each other in a funny way that I could make a meme out of it. There should be relation between them. Generate sentences related to "${topic}". Also give me the category of the relation they are related.

Example:
{"top":"Girls after makeup","bottom":"Girls before makeup","category":"before_after"}
`.trim();

                    let topText, bottomText, category;
                    ...
                    try {
                        const resp = await fetch("https://api.cohere.ai/v1/generate", {
                            ...
                            body: JSON.stringify({
                                model: "command",               // Avoid "command-r" unless you're on the paid plan with access
                                prompt,
                                max_tokens: 80,                 // Enough for one JSON object
                                temperature: 0.9,
                                stop_sequences: ["}"],          // Help model finish cleanly
                                k: 0                            // Turn off randomness sampling
                            })
                        });

                        if (!resp.ok) {
                            const err = await resp.json().catch(() => ({}));
                            throw new Error(`Cohere error ${resp.status}: ${err.message || 'Bad request'}`);
                        }

                        const json = await resp.json();
                        let raw = json.generations?.[0]?.text?.trim();
                        if (!raw) throw new Error("No text generated");

                        // Extract just the {...} portion in case extra stuff is added
                        const firstBrace = raw.indexOf("{");
                        const lastBrace = raw.lastIndexOf("}");
                        const clean = raw.slice(firstBrace, lastBrace + 1);

                        ({ top: topText, bottom: bottomText, category } = JSON.parse(clean));

                        if (!topText || !bottomText || !category)
                            throw new Error("Missing one of top/bottom/category");

                    } catch (err) {
                        console.warn("⚠️ Cohere fallback used:", err.message);
                        topText = "Before async/await";
                        bottomText = "After understanding promises";
                        category = "before_after";
                    }


                    /* ---------- 2️⃣ Pick a template that fits the category -------- */
                    const template_id = chooseTemplate(category);

                    /* ---------- 3️⃣ Call Imgflip to caption the image ------------- */
                    ...
                    /* ---------- 4️⃣ Show pop-up & spend tokens -------------------- */
                    const pop = document.createElement("div");
                    pop.style.cssText = `
     position: fixed; bottom: 20px; right: 20px; z-index: 10002;
     background:#fff;border:2px solid #000;border-radius:10px;
     padding:12px;box-shadow:2px 2px 10px rgba(0,0,0,.35);
     max-width:280px;text-align:center;font-family:sans-serif;`;
                    pop.innerHTML = `
     <strong>🎉 Meme Unlocked!</strong><br>
     <img src="${memeJson.data.url}" style="max-width:100%;border-radius:6px;margin-top:10px"/><br>
     <button style="margin-top:8px;padding:4px 10px;border:none;background:#f44336;color:#fff;border-radius:4px;cursor:pointer;">Close</button>`;
                    pop.querySelector("button").onclick = () => pop.remove();
                    document.body.appendChild(pop);
                    addTokens(-5); // spend token
                };
